...
async function fillDialog(data) {
  ...
  // add runtime
  document.querySelector(".maturity-number").nextElementSibling.innerText =
  ...
  cast.cast.slice(0, 5).forEach((actor, i) => {
    castContainer.innerHTML += `<a href="">${actor.name}${
      i == 4 ? "" : ", "
    }</a>`;
  });
  // genres
  ...
  data.genres.forEach((genre, i) => {
    genresContainer.innerHTML += `<a href="">${genre.name}${
